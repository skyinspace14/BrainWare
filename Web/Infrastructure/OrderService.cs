...
namespace Web.Infrastructure
{
    ...
    public class OrderService
    {
        ...
        public List<OrderDetails> GetCompanyOrders(int CompanyId)
        {
            // Get the orders
            var sql1 =
                "SELECT O.description AS [Description], P.name AS ProductName, OP.price AS ProductPrice,	OP.quantity AS ProductQuantity FROM OrderProduct OP INNER JOIN Product P ON OP.product_id = P.product_id INNER JOIN [Order] O ON OP.order_id = O.order_id WHERE O.company_id = " + CompanyId;
            ...
            while (reader1.Read())
            {
                ...
                {
                    OrderDescription = record1.GetString(0),
                    ProductName = record1.GetString(1),
                    ProductPrice = record1.GetDecimal(2),
                    ProductQuantity = record1.GetInt32(3),
                });
